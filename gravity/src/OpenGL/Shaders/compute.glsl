...
void main() {
	...
	float theta = 0.2f;
	float cx = (gl_GlobalInvocationID.x * 1.0f - 600.0f * 0.125f) * cos(theta) - (gl_GlobalInvocationID.y * 1.0f - 200.0f * 0.5f) * sin(theta);
	float cy = (gl_GlobalInvocationID.x * 1.0f - 600.0f * 0.125f) * sin(theta) + (gl_GlobalInvocationID.y * 1.0f - 200.0f*0.5f) * cos(theta);
	float dx = (cx) * 0.006f;
	float data = datax[gl_GlobalInvocationID.x + gl_GlobalInvocationID.y * n];
	vec4 color =  w >= 1.0f ? color_map(datax[gl_GlobalInvocationID.x + gl_GlobalInvocationID.y * n]) : color_map(datax[gl_GlobalInvocationID.x + gl_GlobalInvocationID.y * n]);

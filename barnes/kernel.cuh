...
__device__ void equilibrium(cell* cell, vec2* e, float* w) {
    ...
    float c = (e[1].x * e[1].x + e[1].y * e[1].y);
    for (int i = 0; i < 9; i++) {
        float cdotu = cell->v.x * e[i].x + cell->v.y * e[i].y;
        cell->fe[i] = cell->rho * w[i] * (1.0f + 3.0f * cdotu / c + 4.5f * cdotu * cdotu / (c * c) - 1.5f * udotu / (c));
    }
}
...
__global__ void macro(cell* cells, vec2* e, float* buffer, int n) {
    ...
    buffer[idx + idy * n] = -.15f + heatmap_velocity * ((cells[idx + idy * n].v.x * cells[idx + idy * n].v.x + cells[idx + idy * n].v.y * cells[idx + idy * n].v.y));
    buffer[idx + idy * n] *= 2;
}
...
__global__ void inflow2(cell* cells, int n) {
  ...
  /*  cells[idy * n].f[1] = cells[idy * n + 1].f[1];
    cells[idy * n].f[5] = cells[idy * n + 1].f[5];
    cells[idy * n].f[8] = cells[idy * n+ 1].f[8];*/
}
...
__global__ void gradient(cell* cells, float* buffer, int n) {
    ...
    float da = cells[idx + 1 + idy * n].rho - cells[idx - 1 + idy * n].rho;
    float db = cells[idx  + (idy + 1) * n].rho - cells[idx + (idy + 1) * n].rho;
    buffer[idx + n * idy] = (cells[idx + (idy ) * n].rho - 0.00997f) * 2000;// heatmap_curl* (dx - dy);
}
